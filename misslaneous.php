        ...
        <div class="row">
            <div class="col-md-12 p-1">
                <div style="border: solid 3px; color:black">
                    <h2 style="background-color: black;color:white;text-align:center;font-weight: 700;">Misslaneous Jobs
                    </h2>
                    <?php
      $data = mysqli_query($con,"SELECT * FROM `misslaneous_link` ORDER BY m_id desc  ");
      while($row=mysqli_fetch_array($data, MYSQLI_ASSOC))
      {
        $m_id = $row["m_id"];
        $cat_name = $row["cat_name"];
        $miss_link = $row["miss_link"];
        $miss_name = $row["miss_name"];
        $miss_details = $row["miss_details"];
      ?>
                    <ul>

                        <li> <a href="misslaneous_details.php?c_id=<?php echo $m_id; ?>"
                                style="font-size:20px;font-weight:600"><?php echo $miss_name; ?></a>
                        </li>
